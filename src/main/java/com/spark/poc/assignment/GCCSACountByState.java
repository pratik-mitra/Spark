...
public class GCCSACountByState {
	...
	public static void main(String[] args) {
		...
		JavaPairRDD<String, Integer> statePairDataSet = filteredStateDataSet.mapToPair(
															(PairFunction<String, String, Integer>)line ->
																	new Tuple2<>(line.split(",")[2],1));		
		JavaPairRDD<String, Integer> singleStatePairDataSet = statePairDataSet.reduceByKey(
														(Function2<Integer, Integer, Integer>) (x, y) -> x + y);
		
		Map<String, Integer> stateDataMap = singleStatePairDataSet.collectAsMap();
		for(Map.Entry<String, Integer> entry : stateDataMap.entrySet()) {
